...
export function TransactionInsightModal({ 
  ...
  const generateInsight = async () => {
    ...
    setLoading(true);
    try {
      const utilization = transaction.utilizationPercentage || 0;
      const zone = transaction.zone || 'Unknown';
      ...
      if (zone === 'Danger' || utilization > 30) {
        generatedInsight = {
          ...
          severity: 'urgent',
          message: `After this ${transaction.amount >= 0 ? 'purchase' : 'payment'} on ${new Date(transaction.date).toLocaleDateString()}, your balance ${transaction.amount >= 0 ? 'increased' : 'decreased'} by $${Math.abs(transaction.amount).toFixed(2)}. ${utilization > 30 ? `This put you at ${utilization.toFixed(1)}% utilization, which is in the danger zone and may hurt your credit score.` : 'Monitor your spending carefully.'}`,
          metadata: {
        ...
        generatedInsight = {
          ...
          severity: 'medium',
          message: `This transaction on ${new Date(transaction.date).toLocaleDateString()} ${transaction.amount >= 0 ? 'increased' : 'decreased'} your balance by $${Math.abs(transaction.amount).toFixed(2)}. ${utilization > 0 ? `Your utilization was ${utilization.toFixed(1)}%, which is in the caution zone. Try to keep it below 30%.` : ''}`,
          metadata: {
            current_utilization: utilization,
            threshold_percentage: 30
          }
        ...
        generatedInsight = {
          ...
          severity: 'success',
          message: `Great! This ${transaction.amount >= 0 ? 'purchase' : 'payment'} on ${new Date(transaction.date).toLocaleDateString()} ${transaction.amount >= 0 ? 'is' : 'helped'} keep your utilization ${utilization > 0 ? `at a safe ${utilization.toFixed(1)}%` : 'low'}. ${transaction.amount < 0 ? 'Payments like this help maintain a healthy credit profile.' : 'Keep your spending in check!'}`,
          metadata: {
            current_utilization: utilization
          }
        };
