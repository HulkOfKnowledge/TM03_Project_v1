...
export default function CardOffersPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {offers.map((offer) => (
                <CardOfferCard
                  ...
                  compareDisabled={compareIds.size >= MAX_COMPARE && !compareIds.has(offer.id)}
                />
              ))}
            </div>
          )}
      ...
      {compareIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-gray-200 dark:border-gray-800 bg-white/95 dark:bg-gray-950/95 backdrop-blur-md px-4 py-3 shadow-2xl">
          <div className="container mx-auto max-w-7xl flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                Compare ({compareIds.size}/{MAX_COMPARE}):
              </span>
              <div className="flex flex-wrap gap-2">
                {selectedOffers.map((o) => (
                  <div
                    key={o.id}
                    className="flex items-center gap-1.5 rounded-full border border-brand/30 bg-brand/5 dark:bg-brand/10 px-3 py-1"
                  >
                    <span
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-brand px-5 py-2 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
